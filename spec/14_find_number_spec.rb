...
describe FindNumber do
  ...
  describe '#make_guess' do
    ...
    # explained above.
    let (:number_guessing) { RandomNumber.new(0, 16) }
    ...
    subject(:game_one) { described_class.new(5, 9) }
    subject(:game_two) { described_class.new(8, 9) }
    subject(:game_three) { described_class.new(0, 3) }
    subject(:game_four) { described_class.new(3, 3) }
  ...
  describe '#game_over?' do
    context 'when guess and random_number are equal' do
      ...
      let (:random_number) { RandomNumber.new(3, 3) }
      subject(:game) { described_class.new(4, 9, random_number, 3) }
    ...
    let (:random_number) { RandomNumber.new(3, 3) }
      subject(:game) { described_class.new(4, 9, random_number, 4) }
  ...
  describe '#update_range' do
    ...
    let (:random_number) { RandomNumber.new(0, 6) }

    subject(:game_one) { described_class.new(5, 8, number_range, 7) }

    context 'when the guess is 7, min=5, and max=8' do
      it 'updates min to the same value as max' do
        expect { game_one.update_range }.to change { game_one.min }.to eq(game_one.max)
      end

      it 'does not update max' do
        expect { game_one.update_range }.not_to change { game_one.max }
      end
    end
  end
end
